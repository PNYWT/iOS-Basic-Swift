...
extension UIView {
    ...
    @MainActor
    public func pushNavigationToReadNews(sectionName: String = "", modelNews: GlobaleListNews, needTransitionFlip: Bool = false, needAnimation: Bool = true) {
        var title: String = modelNews.title
        if !sectionName.isEmpty {
            title = sectionName
        }
        let goWhere = GlobalNewsReadVC(title: title, urlWebString: modelNews.webview)
